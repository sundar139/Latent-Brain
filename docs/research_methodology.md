...
## Trial-aware extraction precedes event-centered cropping

A dataset-wide `crop_to_min` truncates every trial to the shortest trial's length. That is an
acceptable way to build a rectangular model-input tensor, and it is what the frozen processed
artifacts contain. It is not an acceptable source for event-centered windows.

The reason is that the crop is defined by trial *duration*, while the window is defined by a
behavioral *event*. Nothing forces the event to fall inside the retained prefix. On MC_Maze Large,
trials run 2006 to 4141 source bins, the crop keeps the first 2006, and the peak-speed bin lies
outside that prefix for 40% of trials. Centering a window on a peak that was already deleted would
silently center it on whatever remained, and the resulting "peak-centered" window would be a
different quantity on 40% of trials than on the other 60%.

Therefore the movement-window audit rebuilds a trial-aware representation from the raw assets: a
ragged `list[np.ndarray]` per trial, each of shape `[trial_time, neurons]` with an aligned behavior
array, exact trial lengths, trial identifiers, and deterministic ordering. No global crop is applied
before event detection. Windows are extracted from that representation, and only then rebinned to
the reporting bin size, so summing spike counts across adjacent bins retains every spike inside the
window. The frozen processed artifact is never regenerated by the audit and remains the model-input
contract.

## Global crop_to_min is not disqualified by excluded spikes alone

A large excluded-spike count does not by itself make a global crop unsuitable. Spikes may be
excluded from a long post-movement tail that no evaluation window would ever cover. The question is
whether the crop removes the *behavioral events* the windows are aligned to.

The audit therefore reports both: the excluded spike and bin fractions, and separately the fraction
of trials whose peak speed and movement onset survive inside the retained interval. A crop is
declared suitable for movement-window work only when peak speed and movement onset are inside the
retained interval for every trial. On MC_Maze Large the excluded-spike fraction (0.324) and the
peak-loss fraction (0.400) are different numbers answering different questions, and only the second
one settles the matter.

## Movement onset on long, mostly static trials

Movement onset is the first bin whose speed clears a threshold. Taking that threshold as a quantile
of the trial's own speed samples is only sound when movement occupies a fair share of the trial. On
a full-length Large trial the reach occupies a small minority of bins, so the 70th percentile of
speed falls inside the resting jitter and onset collapses to the first bin: measured across the real
Large trials, onset landed at bin 0 for more than 75% of them, even though the hand's speed at trial
start is about 2.5% of its peak.

The onset threshold is therefore floored at a fraction of each trial's speed range, the same
criterion that defines a "moving" bin for coverage. The floor defaults to zero, so the previously
validated MC_Maze Small behavior is unchanged; the Large audit passes the floor explicitly.

## Window selection uses behavior, never model scores

A window that is chosen because it makes a model look good is not an evaluation window, it is a
result. Selecting on model performance and then reporting model performance on the selected window
is circular, and it silently converts a preprocessing choice into a claim.

Window selection is therefore restricted to behavior and alignment criteria, in this order:

1. clipped-trial fraction at or below the configured maximum,
2. peak speed present for nearly all trials,
3. movement onset present for nearly all trials where the policy aligns to onset,
4. moving-bin coverage above the configured floor,
5. endpoint-direction entropy above the configured floor,
6. a reasonable fixed duration,
7. prefer the transferred MC_Maze Small window when it satisfies every gate,
8. prefer the shorter adequate window over a longer equivalent one.

`selection.use_model_scores_for_selection` must be false, and the recommendation function raises if
it is true. The movement-window audit imports no scoring path, trains nothing, and runs no
cross-validation.

## Separation between window selection and model scoring

Window selection and model scoring are separate stages with separate inputs. Selection consumes
behavior and trial geometry. Scoring consumes the selected window and is run afterwards, under
stratified cross-validation, with invalid controls kept as leakage diagnostics rather than
