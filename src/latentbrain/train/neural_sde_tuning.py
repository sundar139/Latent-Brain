...
from latentbrain.eval.neural_sde_tuning import (
    build_neural_sde_result_row,
    rank_neural_sde_results,
    summarize_neural_sde_tuning,
)
...
def expand_neural_sde_grid(grid: dict[str, list[Any]]) -> list[dict[str, Any]]:
    keys = list(grid)
    values = [list(grid[key]) for key in keys]
    return [dict(zip(keys, combo, strict=True)) for combo in itertools.product(*values)]


def make_neural_sde_run_id(index: int, params: dict[str, Any]) -> str:
    def value(key: str) -> str:
        return str(params[key]).replace(".", "p")

    return (
        f"run_{index:03d}_enc{value('encoder_hidden_dim')}"
        f"_drift{value('drift_hidden_dim')}_diff{value('diffusion_hidden_dim')}"
        f"_lat{value('latent_dim')}_fac{value('factor_dim')}"
        f"_drop{value('input_dropout_rate')}_hw{value('heldout_loss_weight')}"
        f"_kl{value('kl_scale')}_ds{value('diffusion_scale')}"
    )


def _resolve_processed_path(config: dict[str, Any]) -> Path:
    path = resolve_configured_path(str(config["dataset"]["processed_path"]), get_repo_root())
    if not path.exists():
        msg = f"Processed dataset is missing: {path}"
...
def _validate_cuda(config: dict[str, Any]) -> str:
    if str(config["runtime"]["device"]) != "cuda":
        msg = "runtime.device must be cuda for neural-SDE-style tuning"
...
def _load_dataset(config: dict[str, Any]) -> tuple[NeuralDataset, str, int]:
    dataset = load_neural_dataset(_resolve_processed_path(config))
...
def _loss_for_batch(
    ...
    scale = heldin.new_tensor(float(training_config.get("drift_regularization_scale", 0.0)))
    base["loss"] = base["loss"] + scale * smoothness
    base["z0_kl_loss"] = gaussian_kl_standard_normal(output["z0_mean"], output["z0_logvar"])
    base["drift_norm"] = output["drift"].norm(dim=-1).mean()
    base["diffusion_mean"] = output["diffusion"].mean()
    base["drift_regularization_loss"] = smoothness
...
def _load_neural_sde_from_checkpoint(
    ...
    model = NeuralSDE(
        NeuralSDEConfig(
            ...
            diffusion_hidden_dim=int(model_config["diffusion_hidden_dim"]),
...
def _extract_neural_sde_factors(
    model: NeuralSDE,
    dataloaders: dict[str, DataLoader[dict[str, torch.Tensor]]],
    device: torch.device,
    ...
    with torch.no_grad():
        for split_name, loader in dataloaders.items():
            ...
            for batch in loader:
                ...
                for key in ("factors", "rates_hz", "latents", "drift", "diffusion"):
                    target = "model_rates_hz" if key == "rates_hz" else key
...
def _run_neural_sde_evaluation(
    ...
    model = _load_neural_sde_from_checkpoint(
    ...
    extracted = _extract_neural_sde_factors(model, dataloaders, device)
    ...
    for split_name in config["evaluation"]["evaluate_splits"]:
        ...
        diagnostic_rows.append(
            {
                "split": split_key,
                "drift_norm": float(np.linalg.norm(extracted[split_key]["drift"], axis=-1).mean()),
                "diffusion_mean": float(extracted[split_key]["diffusion"].mean()),
...
def _run_metrics(run_dir: Path, device: str) -> dict[str, Any]:
    ...
    return {
        ...
        "drift_norm": final.get("drift_norm"),
        "diffusion_mean": final.get("diffusion_mean"),
    }
...
def build_neural_sde_train_config(
...
def _train_and_evaluate_run(
    ...
    model = NeuralSDE(
        NeuralSDEConfig(
            ...
            diffusion_scale=float(run_config["model"]["diffusion_scale"]),
        )
    )
...
def run_neural_sde_tuning(config: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    grid = expand_neural_sde_grid(dict(config["grid"]))[: int(config["search"]["max_runs"])]
    rows: list[dict[str, Any]] = []
    base = copy.deepcopy(config)
    base["_window_bins"] = window_bins
    for run_index, params in enumerate(grid):
        run_id = make_neural_sde_run_id(run_index, params)
        run_dir = output_dir / "runs" / run_id
        run_config = build_neural_sde_train_config(base, params, run_dir)
    ...
    summary = summarize_neural_sde_tuning(results, refs)
    summary.update(
        {
    ...
    if not leaderboard.empty:
        ...
        best_config = build_neural_sde_train_config(
