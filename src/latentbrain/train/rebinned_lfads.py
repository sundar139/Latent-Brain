...
def coordinated_dropout_run_id(dropout_rate: float) -> str:
    return f"dropout_{dropout_rate:.2f}".replace(".", "p")
...
def build_coordinated_dropout_lfads_train_config(
    base_config: dict[str, Any],
    dropout_rate: float,
    window_bins: int,
    output_dir: Path,
) -> dict[str, Any]:
    """Build one 20 ms LFADS-style masked co-smoothing config with input dropout."""
    config = copy.deepcopy(base_config)
    settings = dict(config["lfads_settings"])
    dropout = dict(config["dropout"])
    refs = dict(config["references"])
    run_id = coordinated_dropout_run_id(dropout_rate)
    config["dataset"]["bin_size_ms"] = int(config["binning"]["target_bin_size_ms"])
    config["data"] = {
        "input_neuron_group": "heldin",
        "target_neuron_group": "heldout",
        "max_time_bins": int(window_bins),
    ...
    config["training"] = {
        ...
        "device": str(config["runtime"]["device"]),
        "input_dropout": {
            "enabled": bool(dropout["enabled"]),
            "mode": str(dropout["mode"]),
            "rate": float(dropout_rate),
            "apply_to": list(dropout.get("apply_to", ["train"])),
            "resample_each_batch": bool(dropout.get("resample_each_batch", True)),
            "keep_at_least_one_neuron": bool(dropout["keep_at_least_one_neuron"]),
    ...
    config["evaluation"] = {
        "evaluate_splits": list(base_config["evaluation"]["evaluate_splits"]),
        "primary_split": str(base_config["evaluation"].get("primary_split", "validation")),
        "direct_model_primary": bool(base_config["evaluation"].get("direct_model_primary", True)),
        "also_evaluate_factor_decoder": bool(
            base_config["evaluation"].get("also_evaluate_factor_decoder", True)
        ),
        "behavior_decoder_enabled": bool(
            base_config["evaluation"].get("behavior_decoder_enabled", True)
        ),
        "baseline_references": {
            "window_matched_mean_rate_validation_bits_per_spike": float(
                refs["same_bin_mean_rate_validation_bits_per_spike"]
