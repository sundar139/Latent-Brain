...
def build_neural_ode_result_row(
    run_id: str,
    run_index: int,
    params: dict[str, Any],
...
def summarize_neural_ode_tuning(
    results: pd.DataFrame,
    references: dict[str, float],
) -> dict[str, Any]:
    ...
    return summary | {
        ...
        "beats_previous_neural_sde": bool(best["beats_previous_neural_sde"]),
        "beats_previous_best_lfads_family": bool(best_result["beats_previous_best_lfads_family"]),
        "best_run_params": {
            key: best_result[key]
            for key in (
                "encoder_hidden_dim",
