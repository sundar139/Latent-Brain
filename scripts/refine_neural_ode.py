...
from latentbrain.eval.reporting import write_neural_ode_refinement_outputs
from latentbrain.paths import get_repo_root, resolve_configured_path
from latentbrain.train.neural_ode_refinement import run_neural_ode_refinement

console = Console(markup=False)


def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run local deterministic neural-ODE refinement.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/mc_maze_small_neural_ode_refinement.yaml"),
    )
...
def _load_config(path: Path) -> dict[str, Any]:
    try:
        ...
        msg = f"malformed deterministic neural-ODE refinement config: {path}"
        raise ValueError(msg) from exc
    if not isinstance(raw, dict):
        msg = f"deterministic neural-ODE refinement config must contain a mapping: {path}"
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    if abs(float(config["model"]["dt_seconds"]) - target_seconds) > 1e-12:
        ...
        raise ValueError(msg)
    if str(config["model"].get("name")) != "neural_ode_refinement":
        msg = "model.name must be neural_ode_refinement"
        raise ValueError(msg)
    ...
    if any(float(value) < 0.0 or float(value) >= 1.0 for value in grid["input_dropout_rate"]):
        ...
        raise ValueError(msg)
    for key in ("kl_scale", "drift_regularization"):
        if any(float(value) < 0.0 for value in grid[key]):
            msg = f"grid.{key} values must be non-negative"
            raise ValueError(msg)
    if any(str(value) not in {"none", "cosine"} for value in grid["scheduler"]):
        msg = "grid.scheduler values must be none or cosine"
        raise ValueError(msg)
    ...
    for key in (
        ...
        "previous_switching_ode_validation_bits_per_spike",
...
def _write_figures(output_dir: Path, results: pd.DataFrame, leaderboard: pd.DataFrame) -> None:
    ...
    if not results.empty:
        ...
        fig.tight_layout()
        fig.savefig(figures / "refinement_validation_bits_by_run.png", dpi=150)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(8, 4))
        ax.bar(results["run_id"], results["drift_regularization_loss"])
        ax.set_ylabel("Drift regularization loss")
        ax.tick_params(axis="x", rotation=30)
        fig.tight_layout()
        fig.savefig(figures / "drift_regularization_by_run.png", dpi=150)
        plt.close(fig)

    if not leaderboard.empty:
        best_run = str(leaderboard.iloc[0]["run_id"])
        best_row = results[results["run_id"] == best_run].iloc[0]
        run_dir = Path(str(best_row.get("output_dir")))
        ...
        if history_path.exists():
            ...
            fig.tight_layout()
            fig.savefig(figures / "scheduler_trace_best.png", dpi=150)
            plt.close(fig)
        checkpoint_path = run_dir / "checkpoint_scores.csv"
        if checkpoint_path.exists():
            checkpoints = pd.read_csv(checkpoint_path)
            x_column = "epoch" if "epoch" in checkpoints else "epoch_or_source"
            fig, ax = plt.subplots(figsize=(6, 4))
            ax.plot(
                checkpoints[x_column],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not cuda["cuda_available"]:
        ...
        return 2
    try:
        results, summary = run_neural_ode_refinement(config)
    ...
    write_neural_ode_refinement_outputs(
        output_dir, summary, results, leaderboard, checkpoint_scores
    )
    ...
    for key in (
        ...
        "previous_switching_ode_reference",
