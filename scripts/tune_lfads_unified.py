...
from latentbrain.eval.reporting import write_lfads_unified_tuning_outputs
from latentbrain.paths import get_repo_root, resolve_configured_path
from latentbrain.train.lfads_unified_tuning import run_lfads_unified_tuning

console = Console(markup=False)


def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run local LFADS-style unified tuning.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/mc_maze_small_lfads_unified_tuning.yaml"),
    )
...
def _load_config(path: Path) -> dict[str, Any]:
    try:
        ...
        msg = f"malformed LFADS unified tuning config: {path}"
        raise ValueError(msg) from exc
    if not isinstance(raw, dict):
        msg = f"LFADS unified tuning config must contain a mapping: {path}"
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    if scoring["reference_model"] != "train_heldout_mean_rate":
        ...
        raise ValueError(msg)
    if scoring.get("selection_metric", False):
        msg = "scoring.selection_metric is not supported"
        raise ValueError(msg)
...
def _write_figures(output_dir: Path, results: pd.DataFrame, leaderboard: pd.DataFrame) -> None:
    ...
    if not results.empty:
        ...
        plt.tight_layout()
        plt.savefig(figures / "unified_validation_bits_by_run.png")
        plt.close()

        plt.figure()
        plt.scatter(results["input_dropout_rate"], results["validation_unified_bits_per_spike"])
        plt.xlabel("Input dropout rate")
        plt.ylabel("Validation unified bits/spike")
        plt.tight_layout()
        plt.savefig(figures / "unified_bits_vs_dropout.png")
        plt.close()
    ...
    if not leaderboard.empty:
        ...
        if history_path.exists():
            ...
            plt.figure()
            train_col = (
                "train_total_loss" if "train_total_loss" in history else "training_batch_loss"
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not cuda["cuda_available"]:
        ...
        return 2
    try:
        results, summary = run_lfads_unified_tuning(config)
    ...
    write_lfads_unified_tuning_outputs(output_dir, summary, results, leaderboard)
