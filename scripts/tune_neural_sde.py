...
from latentbrain.eval.reporting import write_neural_sde_tuning_outputs
from latentbrain.paths import get_repo_root, resolve_configured_path
from latentbrain.train.neural_sde_tuning import run_neural_sde_tuning
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run local neural-SDE-style latent generator tuning."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/mc_maze_small_neural_sde_tuning.yaml"),
    )
...
def _load_config(path: Path) -> dict[str, Any]:
    try:
        ...
        msg = f"malformed neural-SDE-style tuning config: {path}"
        raise ValueError(msg) from exc
    if not isinstance(raw, dict):
        msg = f"neural-SDE-style tuning config must contain a mapping: {path}"
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    if any(float(value) < 0.0 or float(value) >= 1.0 for value in grid["input_dropout_rate"]):
        ...
        raise ValueError(msg)
    for key in ("kl_scale", "diffusion_scale"):
...
def _write_figures(output_dir: Path, results: pd.DataFrame, leaderboard: pd.DataFrame) -> None:
    ...
    if not results.empty:
        ...
        fig.tight_layout()
        fig.savefig(figures / "neural_sde_validation_bits_by_run.png", dpi=150)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(6, 4))
        ax.scatter(results["drift_norm"], results["diffusion_mean"])
        ax.set_xlabel("Drift norm")
        ax.set_ylabel("Diffusion mean")
        fig.tight_layout()
        fig.savefig(figures / "drift_diffusion_diagnostics.png", dpi=150)
        plt.close(fig)
    ...
    if not leaderboard.empty:
        ...
        history_path = (
            Path(str(output_value)) / "metrics_history.csv" if output_value is not None else None
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not cuda["cuda_available"]:
        ...
        return 2
    try:
        results, summary = run_neural_sde_tuning(config)
    ...
    write_neural_sde_tuning_outputs(output_dir, summary, results, leaderboard)
