...
from latentbrain.eval.reporting import write_switching_ode_tuning_outputs
...
from latentbrain.train.switching_ode_tuning import run_switching_ode_tuning
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run local switching neural-ODE-style latent dynamics tuning."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/mc_maze_small_switching_ode_tuning.yaml"),
    )
...
def _load_config(path: Path) -> dict[str, Any]:
    try:
        ...
        msg = f"malformed switching neural-ODE-style tuning config: {path}"
        raise ValueError(msg) from exc
    if not isinstance(raw, dict):
        msg = f"switching neural-ODE-style tuning config must contain a mapping: {path}"
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    if abs(float(config["model"]["dt_seconds"]) - target_seconds) > 1e-12:
        ...
        raise ValueError(msg)
    if str(config["model"].get("name")) != "switching_ode":
        msg = "model.name must be switching_ode"
        raise ValueError(msg)
    if float(config["model"].get("diffusion_scale", 0.0)) != 0.0:
        msg = "model.diffusion_scale must be exactly 0.0"
        raise ValueError(msg)
    ...
    for key in (
        ...
        if any(int(value) <= 0 for value in grid[key]):
            ...
            raise ValueError(msg)
    if any(int(value) < 2 for value in grid["n_regimes"]):
        msg = "grid.n_regimes values must be at least 2"
        raise ValueError(msg)
    if any(float(value) <= 0.0 for value in grid["regime_temperature"]):
        msg = "grid.regime_temperature values must be positive"
        raise ValueError(msg)
    if any(float(value) < 0.0 or float(value) >= 1.0 for value in grid["input_dropout_rate"]):
        ...
        raise ValueError(msg)
    for key in ("kl_scale", "entropy_regularization"):
    ...
    for key in (
        ...
        "previous_neural_ode_validation_bits_per_spike",
...
def _write_figures(output_dir: Path, results: pd.DataFrame, leaderboard: pd.DataFrame) -> None:
    ...
    if not results.empty:
        ...
        fig.tight_layout()
        fig.savefig(figures / "switching_validation_bits_by_run.png", dpi=150)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(8, 4))
        ax.bar(results["run_id"], results["max_regime_occupancy"])
        ax.set_ylabel("Max regime occupancy")
        ax.tick_params(axis="x", rotation=30)
        fig.tight_layout()
        fig.savefig(figures / "regime_occupancy_by_run.png", dpi=150)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(8, 4))
        ax.bar(results["run_id"], results["mean_regime_entropy"])
        ax.set_ylabel("Mean regime entropy")
        ax.tick_params(axis="x", rotation=30)
        fig.tight_layout()
        fig.savefig(figures / "regime_entropy_by_run.png", dpi=150)
        plt.close(fig)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not cuda["cuda_available"]:
        ...
        return 2
    try:
        results, summary = run_switching_ode_tuning(config)
    ...
    regime_diagnostics = pd.read_csv(output_dir / "regime_diagnostics.csv")
    write_switching_ode_tuning_outputs(
        output_dir, summary, results, leaderboard, checkpoint_scores, regime_diagnostics
    )
    ...
    for key in (
        ...
        "best_active_regime_count",
        "best_mean_regime_entropy",
        "best_max_regime_occupancy",
        "best_checkpoint_source",
        "factor_latent_unified_reference",
        "previous_neural_ode_reference",
